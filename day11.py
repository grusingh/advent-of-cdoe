...
def get_matrix(lines):
    ...
    for idx in range(len(empty_cols_idx)):
        ci = empty_cols_idx[idx]
        for ri in range(len(rows)):
            rows[ri].insert(ci + idx, ".")

    new_rows = []
    for r in rows:
        new_rows.append(r)
        if len(set(r)) == 1 and r[0] == ".":
            # print("found empty row", len(rows))
            new_rows.append(r)
